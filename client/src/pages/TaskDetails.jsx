...
const TaskDetails = () => {
  ...
  useEffect(() => {
    ...
    socket.on("comment_added", (newCommentData) => {
      if (newCommentData.taskId === taskId) {
        setComments((prev) => {
          // Avoid duplicates if comment was added by current user
          const exists = prev.some((c) => c.id === newCommentData.id);
          return exists ? prev : [...prev, newCommentData];
        });
      }
    });
