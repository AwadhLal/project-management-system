...
import { XIcon, Users, Plus } from "lucide-react";
...
const CreateProjectDialog = ({ isDialogOpen, setIsDialogOpen }) => {
  ...
  // Fetch company employees if user is company admin
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const projectData = {
        ...
        ...formData,
        // Include selected company members if admin
        companyMembers: user?.userType === "COMPANY_ADMIN" ? selectedMembers : []
      };
  ...
  const handleMembersUpdated = (memberIds) => {
      ...
      <div className="fixed inset-0 bg-black/20 dark:bg-black/60 backdrop-blur flex items-center justify-center text-left z-50">
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 w-full max-w-lg text-zinc-900 dark:text-zinc-200 relative max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Company Members Section (Only for Company Admins) */}
            {user?.userType === "COMPANY_ADMIN" && companyEmployees.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm">Company Team Members</label>
                  <button
                    type="button"
                ...
                {selectedMembers.length > 0 && (
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    <p className="text-sm text-blue-800 dark:text-blue-200 mb-2">
                      ...
                      {selectedMembers.length} company member(s) selected
                    </p>
                    <div className="flex flex-wrap gap-1">
                      {selectedMembers.map((memberId) => {
                        const employee = companyEmployees.find(emp => emp.id === memberId);
                        return employee ? (
                          <span
                            key={memberId}
                            className="inline-flex items-center px-2 py-1 rounded text-xs bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-200"
                          >
                            {employee.name}
                          </span>
                        ) : null;
            ...
            {/* Team Members */}
            <div>
              <label className="block text-sm mb-1">Workspace Team Members</label>
              <select
                className="w-full px-3 py-2 rounded dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 mt-1 text-zinc-900 dark:text-zinc-200 text-sm"
                onChange={(e) => {
                  if (
              ...
              {formData.team_members.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.team_members.map((email) => (
                    <div
                      key={email}
                      className="flex items-center gap-1 bg-green-200/50 dark:bg-green-500/20 text-green-700 dark:text-green-400 px-2 py-1 rounded-md text-sm"
                    >
                      {email}
                      <button
                        type="button"
                        onClick={() => removeTeamMember(email)}
                        className="ml-1 hover:bg-green-300/30 dark:hover:bg-green-500/30 rounded"
                      >
